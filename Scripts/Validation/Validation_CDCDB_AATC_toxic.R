# Validation using the CDCDB drug combinations (AATC, toxic)

# Notes:
# (1) The two drug combinations from AATC, with reported toxicity 
...
# Read the AATC drug combination from CDCDB
drugCombs <- readRDS("InputFiles/ReferenceList/CDCDB_drugCombinations_AATC_toxic.rds")
...
drugCombs$name <- paste("Unk", drugCombs$Drug1_DrugBank_drug_id, drugCombs$Drug2_DrugBank_drug_id, sep = "__")
row.names(drugCombs) <- NULL
...
for(disease in c("LungCancer", "BreastCancer", "ProstateCancer", "OvaryCancer", "KidneyCancer", "SkinCancer")){
  ...
  predictions$nct_id <- drugCombs$nct_id[match(predictions$drugCombs, drugCombs$name)]
  predictions$condition <- drugCombs$condition[match(predictions$drugCombs, drugCombs$name)]
...
write.xlsx(validation_predictions, "OutputFiles/Validation/CDCDB/validation_CDCDB_AATC_toxic.xlsx", rowNames = FALSE, overwrite = TRUE)
...
tiff("OutputFiles/Validation/CDCDB/validation_CDCDB_AATC_toxic.tiff", 
     width = 6, height = 5, 
...
ggplot(validation_predictions, aes(x = Disease, y = Eff, fill = Class)) + #
  ...
  ggtitle(paste0("Predictions for C-DCDB drug combinations (AATC, toxic)")) +
