# Validation using the CDCDB drug combinations (DISCN type)
...
drugCombs <- readRDS("InputFiles/ReferenceList/CDCDB_drugCombinations_OrangeBook_discn.rds")
...
write.xlsx(validation_predictions, "OutputFiles/Validation/CDCDB/validation_CDCDB_OrangeBook_discn.xlsx", rowNames = FALSE, overwrite = TRUE)
...
tiff("OutputFiles/Validation/CDCDB/validation_CDCDB_OrangeBook_discn.tiff", 
     width = 6, height = 5, 
...
ggplot(validation_predictions, aes(x = Disease, y = Eff, fill = Class)) + #
  ...
  ggtitle(paste0("Predictions for C-DCDB drug combinations (Orange Book, DISCN)")) +
