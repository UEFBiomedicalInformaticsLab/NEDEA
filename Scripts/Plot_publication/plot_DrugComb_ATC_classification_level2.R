...
# Script to plot the level_2 ATC codes as heatmap
...
drugCombs[is.na(drugCombs$Drug1_ATC_code_2), "Drug1_ATC_code_2"] <- "MISSING"
drugCombs[is.na(drugCombs$Drug2_ATC_code_2), "Drug2_ATC_code_2"] <- "MISSING"
...
all_atc <- sort(unique(c(drugCombs$Drug1_ATC_code_2, drugCombs$Drug2_ATC_code_2)))
  ...
  for(disease in c("BreastCancer", "KidneyCancer", "LungCancer", "OvaryCancer", "ProstateCancer", "SkinCancer")){
    ...
    if(nrow(drugCombs_select) > 1){
      for(i in 1:nrow(drugCombs_select)){
        ...
        atc_1 <- drugCombs_select[i, "Drug1_ATC_code_2"]
        atc_2 <- drugCombs_select[i, "Drug2_ATC_code_2"]
...
tiff(paste0("OutputFiles/Plots_publication/ATC_classification/drugCombs_ATCclass_level2.tiff"),
     width = 29,
     height = 25,
