...
# Script to compare the ATC codes (level 1) of the drugs forming various datasets
  ...
  for(disease in c("BreastCancer", "KidneyCancer", "LungCancer", "OvaryCancer", "ProstateCancer", "SkinCancer")){
    ...
    switch(dataset, 
           "Training" = {
             ...
             ATC_master[[disease]][[dataset]] <- sort(na.exclude(unique(c(drugCombs$Drug1_ATC_code_1, drugCombs$Drug2_ATC_code_1))))
           },
...
tiff(paste0("OutputFiles/Plots_publication/ATC_classification/drugCombs_ATCclass_compare_level1.tiff"),
     width = 8,
     height = 6,
