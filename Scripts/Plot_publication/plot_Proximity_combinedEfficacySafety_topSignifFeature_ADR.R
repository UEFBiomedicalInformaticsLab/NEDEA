...
for(disease in c("BreastCancer", "KidneyCancer", "LungCancer", "OvaryCancer", "ProstateCancer", "SkinCancer")){
  ...
  for(drug_target_type in c("known", "KEGG", "NPA", "PS", "RI", "SIGNOR","all")){
    ...
    for(lib_name in unique(stat_data$feature)){
      ...
      stat_res <- wilcox.test(x = stat_data_select$value[stat_data_select$category == "adr_positive"],
                              y = stat_data_select$value[stat_data_select$category == "unknown"])
  ...
  if(!dir.exists(paste0("OutputFiles/Plots_publication/Proximity", proximity_type, "_", feature_type, "_boxplot_topSignifFeatures_ADR/"))){
    dir.create(paste0("OutputFiles/Plots_publication/Proximity", proximity_type, "_", feature_type, "_boxplot_topSignifFeatures_ADR/"))
  }
  
  tiff(paste0("OutputFiles/Plots_publication/Proximity", proximity_type, "_", feature_type, "_boxplot_topSignifFeatures_ADR/Proximity", proximity_type, "_", feature_type, "_boxplot_topSignifFeatures_ADR_", disease, ".tiff"), 
       width = 20,
       height = 7,
