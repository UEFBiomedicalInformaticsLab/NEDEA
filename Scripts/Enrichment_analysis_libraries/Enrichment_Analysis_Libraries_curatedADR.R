...
ADReCS_curatedAdr_lib <- ADReCS_ADR2Gene_lib[names(ADReCS_ADR2Gene_lib) %in% ADR_terms]
names(ADReCS_curatedAdr_lib) <- paste0(names(ADReCS_curatedAdr_lib), " [ADReCS]")
...
CTD_curatedAdr_lib <- CTD_Disease2Gene_lib[names(CTD_Disease2Gene_lib) %in% ADR_terms]
names(CTD_curatedAdr_lib) <- paste0(names(CTD_curatedAdr_lib), " [CTD]")
...
DisGeNET_curatedAdr_lib <- DisGeNET_Disease2Gene_lib[names(DisGeNET_Disease2Gene_lib) %in% ADR_terms]
names(DisGeNET_curatedAdr_lib) <- paste0(names(DisGeNET_curatedAdr_lib), " [DisGeNET]")
...
# 
# OpenTargets_curatedAdr_lib <- OpenTargets_Disease2Gene_lib[names(OpenTargets_Disease2Gene_lib) %in% ADR_terms]
# names(OpenTargets_curatedAdr_lib) <- paste0(names(OpenTargets_curatedAdr_lib), " [OpenTargets]")



# Compile all gene list into one list
curatedAdr2Gene_lib <- c(ADReCS_curatedAdr_lib,
                         CTD_curatedAdr_lib,
                         DisGeNET_curatedAdr_lib)
...
saveRDS(curatedAdr2Gene_lib, "InputFiles/Enrichment_analysis_libraries/curatedAdr2Gene_lib.rds")
...
lib_size <- as.data.frame(lengths(curatedAdr2Gene_lib))
...
for (i in unique(names(curatedAdr2Gene_lib))) {
  for (j in unique(names(curatedAdr2Gene_lib))) {
    if (i != j) {
      intersection <-
        intersect(curatedAdr2Gene_lib[[i]], curatedAdr2Gene_lib[[j]])
      union <- union(curatedAdr2Gene_lib[[i]], curatedAdr2Gene_lib[[j]])
...
write.xlsx(list(Library_size = lib_size,
           ...
           "OutputFiles/Tables/curatedAdr2Gene_libInfo.xlsx", overwrite = TRUE)
