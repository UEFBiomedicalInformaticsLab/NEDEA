...
# Script to check the overlap score between drug targets (known and extended) with the compiled efficacy and safety library
...
for(drug_target_type in names(result_final)){
  write.xlsx(x = result_final[[drug_target_type]],
             file = paste0("OutputFiles/Geneset_overlap_check/OverlapScore_", drug_target_type, "_target_enrichLib.xlsx"),
             overwrite = TRUE)
}
saveRDS(result_final, "OutputFiles/Geneset_overlap_check/OverlapScore_target_enrichLib.rds")
