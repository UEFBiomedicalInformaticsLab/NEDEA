...
# Script to check the overlap between genes selected for FGSEA with the extended efficacy and safety library
  ...
  foreach(disease = c("BreastCancer", "KidneyCancer", "LungCancer", "OvaryCancer", "ProstateCancer", "SkinCancer"), 
          ...
          .packages = c("tidyverse", "org.Hs.eg.db")) %dopar% {
            ...
            for(drugComb in names(drugCombs_targets)){
              ...
              rankedGeneList <- names(rwr_data_select[rwr_data_select > rwr_threshold$ELB])
              
              tmp1 <- lapply(enrichment_lib, function(x){
                round((length(intersect(x, rankedGeneList))/length(x)) * 100, 3)
              })
              
              result <- rbind(result, data.frame("comb_name" = drugComb,
                                                 "Number_of_targets" = length(target_set),
                                                 "Number_of_genes_for_fgsea" = length(rankedGeneList),
                                                 rbind(unlist(tmp1)),
                                                 check.names = FALSE,
                                                 row.names = NULL))
...
for(drug_target_type in names(result_final)){
  write.xlsx(x = result_final[[drug_target_type]], 
             file = paste0("OutputFiles/Geneset_overlap_check/OverlapPerc_", drug_target_type, "_rankedGene_extendedEnrichLib.xlsx"), 
             overwrite = TRUE)
}
saveRDS(result_final, "OutputFiles/Geneset_overlap_check/OverlapPerc_rankedGene_extendedEnrichLib.rds")
