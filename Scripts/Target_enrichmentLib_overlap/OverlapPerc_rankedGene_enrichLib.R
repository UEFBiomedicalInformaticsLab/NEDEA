...
# Script to check the overlap between genes selected for FGSEA with the compiled efficacy and safety library
...
for(drug_target_type in names(result_final)){
  write.xlsx(x = result_final[[drug_target_type]], 
             file = paste0("OutputFiles/Geneset_overlap_check/OverlapPerc_", drug_target_type, "_rankedGene_enrichLib.xlsx"), 
             overwrite = TRUE)
}
saveRDS(result_final, "OutputFiles/Geneset_overlap_check/OverlapPerc_rankedGene_enrichLib.rds")
