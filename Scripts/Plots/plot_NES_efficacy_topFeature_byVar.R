...
# Script to plot the NES of top (by variance) features from FGSEA of efficacy library
...
for(drug_target_type in c("known", "PS", "SIGNOR", "NPA", "RI", "KEGG", "all")){
  for(disease in c("BreastCancer", "KidneyCancer", "LungCancer", "OvaryCancer", "ProstateCancer", "SkinCancer")){
    ...
    fgsea_result <- fgsea_result$efficacy
...
tiff("OutputFiles/Plots/top_feature_NES_scatter/plot_NES_efficacy_topFeature_byVar.tiff",
     width = 30, height = 25,
