...
func_train_model <- function(feature_matrix, 
  ...
  res <- foreach(i=names(train_test_split), 
                 ...
                 .export = c("train_rf_model", "train_glmnet_model", "train_svmRadial_model", "train_nb_model")) %dopar% {
                   ...
                   rf_result_table <- data.frame(Fold = i,
                                                 trainClass_count,
                                                 testClass_count,
                   ...
                   if(ncol(trainData) > 1){
                     ...
                     glmnet_result_table <- data.frame(Fold = i,
                                                       trainClass_count,
                                                       testClass_count,
                   ...
                   svmRadial_result_table <- data.frame(Fold = i,
                                                        trainClass_count,
                                                        testClass_count,
                   ...
                   nb_result_table <- data.frame(Fold = i,
                                                 trainClass_count,
                                                 testClass_count,
                   ...
                   # Compile results
                   tmp1 <- list(rf = list(model = rf_model, 
  ...
  for(fold in names(res)){
    modelling_results[[fold]] <- res[[fold]]$modelling_results 
    result_summary_tables[[fold]] <- res[[fold]]$result_summary_tables
  }
  
  tmp1 <- list()
  for(model_select in unique(unlist(lapply(result_summary_tables, names), use.names = FALSE))){
    tmp1[[model_select]] <- do.call(rbind, lapply(result_summary_tables, function(x){x[[model_select]]}))
  }
  return(list(result_summary_tables = tmp1, 
