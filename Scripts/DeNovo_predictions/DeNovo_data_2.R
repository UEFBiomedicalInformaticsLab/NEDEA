...
# Script to generate drug combinations for de novo predictions (test)
...
# Read the drug group
DrugBank_drug_group <- readRDS("Databases/DrugBank/parsed_DrugBank_data.rds")
DrugBank_drug_group <- DrugBank_drug_group$drugs$groups
tmp1 <- unique(DrugBank_drug_group$group)
DrugBank_drug_group <- DrugBank_drug_group %>% 
  group_by(`drugbank-id`) %>% 
  summarise(group = paste(sort(group), collapse = ","))

DrugBank_drug_group <- DrugBank_drug_group[grep("withdrawn|nutraceutical|illicit", 
                                                DrugBank_drug_group$group, 
                                                invert = TRUE), ]

DrugBank_drug <- readRDS("Databases/DrugBank/parsed_DrugBank_data.rds")
DrugBank_drug <- DrugBank_drug$drugs$general_information
  
  
# Remove drugs of following categories: amino acids, metals
DrugBank_drug_categories <- readRDS("Databases/DrugBank/parsed_DrugBank_data.rds")
DrugBank_drug_categories <- DrugBank_drug_categories$drugs$categories

remove_drugs <- DrugBank_drug_categories[DrugBank_drug_categories$category %in% c("Amino Acids", "Metals", "Metal cations"), ]


# Read drug target interactions
drug_target_ixn <- readRDS("InputFiles/Associations/DrugBank_Drug_Target_associations.rds")
drug_target_ixn <- drug_target_ixn[drug_target_ixn$drugbank_drug_id %in% DrugBank_drug_type$primary_key & 
                                     drug_target_ixn$drugbank_drug_id %in% DrugBank_drug_group$`drugbank-id`, ]
drug_target_ixn <- drug_target_ixn[!drug_target_ixn$drugbank_drug_id %in% remove_drugs$parent_key, ]
...
tmp1 <- drug_target_ixn %>% group_by(drugbank_drug_id) %>% 
  summarise(target_count = length(unique(ensembl_gene_id))) %>% 
  filter(target_count >= 5)


# Aggregate the drug targets as a single string
drugTarget_list <- drug_target_ixn %>%
  filter(drugbank_drug_id %in% tmp1$drugbank_drug_id) %>% 
...
denovo_drugCombs <- as.data.frame(t(combn(drugTarget_list$drugbank_drug_id, m = 2, simplify = TRUE)))
...
CHG_pathways <- readxl::read_excel("Databases/CHG/Table_1.xls", skip = 2)
...
if(!dir.exists("InputFiles/DeNovo_data_2/")){dir.create("InputFiles/DeNovo_data_2/", recursive = TRUE)}
saveRDS(denovo_drugCombs, file = paste0("InputFiles/DeNovo_data_2/drugCombs_denovo2_", disease, ".rds"))
