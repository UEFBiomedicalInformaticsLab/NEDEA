...
# Use the trained model to predict the labels on the de novo data 2
...
denovo_drugCombs <- readRDS(file = paste0("InputFiles/DeNovo_data_2/drugCombs_denovo2_", disease, ".rds"))
...
denovo_fgsea_result <- readRDS(file = paste0("OutputFiles/DeNovo_data_2/Features/fgseaNES_combinedEfficacySafety_", disease, "_", drug_target_type, ".rds"))
...
if(!dir.exists("OutputFiles/DeNovo_data_2/Predictions/")){ dir.create("OutputFiles/DeNovo_data_2/Predictions/", recursive = TRUE) }
write.csv(predict_result, file = paste0("OutputFiles/DeNovo_data_2/Predictions/predictions_NES_combinedEfficacySafety_", disease, "_", drug_target_type, ".csv"), row.names = FALSE)
...
DrugBank_drug_ATC_tmp1 <- DrugBank_drug_ATC[, c("DrugBank_drug_ID", "level_1", "atc_code")]
DrugBank_drug_ATC_tmp1 <- DrugBank_drug_ATC_tmp1 %>% 
...
priority_drugCombs <- priority_drugCombs %>%
  left_join(DrugBank_drug_ATC_tmp1 %>% rename_with(.cols = everything(),
                                                   .fn = ~ paste0("Drug1_", .)),
...
if(!dir.exists("OutputFiles/DeNovo_data_2/Priority_drug_combinations/")){ dir.create("OutputFiles/DeNovo_data_2/Priority_drug_combinations/", recursive = TRUE) }
write.csv(priority_drugCombs, file = paste0("OutputFiles/DeNovo_data_2/Priority_drug_combinations/priorityDrugCombs_NES_combinedEfficacySafety_", disease, "_", drug_target_type, ".csv"), row.names = FALSE)
...
drugCombs <- priority_drugCombs[, c("Drug1_DrugBank_id", "Drug2_DrugBank_id")]
drugCombs <- drugCombs %>% 
...
if(!dir.exists("OutputFiles/Plots/ATC_classification/DeNovo_data_2/")){
  dir.create("OutputFiles/Plots/ATC_classification/DeNovo_data_2/", recursive = TRUE)
}

tiff(paste0("OutputFiles/Plots/ATC_classification/DeNovo_data_2/priority_drugCombs_ATCclass_", disease, ".tiff"),
     width = 8,
     height = 6,
...
tiff(paste0("OutputFiles/Plots/DeNovo_predictions/plot_DeNovo_2_predictions_combinedEfficacySafety_", disease, "_", drug_target_type, ".tiff"),
     width = 7, height = 6,
     units = "cm", compression = "lzw", res = 1200)
