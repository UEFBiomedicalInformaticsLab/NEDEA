...
# Get the ATC codes for the drugs in training
training_ATC <- DrugBank_drug_ATC %>% 
  filter(DrugBank_drug_ID %in% unique(c(train_drugCombs_cat$Drug1_DrugBank_id, train_drugCombs_cat$Drug2_DrugBank_id)))


#Add the ATC codes to the validation drug combinations
...
# Filter the drug combinations with include ATC codes from training
valid_drugCombs_cat <- valid_drugCombs_cat %>%
  filter(Drug1_ATC_code_1 %in% training_ATC$code_1 &
           Drug2_ATC_code_1 %in% training_ATC$code_1)

valid_drugCombs_cat <- valid_drugCombs_cat %>% 
  dplyr::select(!c(Drug1_ATC_code_1, Drug2_ATC_code_1)) %>% 
  distinct()
