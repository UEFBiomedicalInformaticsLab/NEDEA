...
# Script for plotting the model accuracy (for publication: comparison of efficay vs safety vs combined)
...
for(disease in c("ProstateCancer")){
...
# saveRDS(model_stats, "OutputFiles/Plots/Publication/model_stats_4.rds")
# model_stats <- readRDS("OutputFiles/Plots/Publication/model_stats_4.rds")
...
select_model_stats$featureType <- gsub("Dis2Gene", "Efficacy \n (RWR-FGSEA)", select_model_stats$featureType)
select_model_stats$featureType <- gsub("WdrlAdr2Gene", "Safety \n (RWR-FGSEA)", select_model_stats$featureType)
select_model_stats$featureType <- gsub("CombDisAdr2Gene", "Combined-efficacy-safety \n (RWR-FGSEA)", select_model_stats$featureType)
select_model_stats$featureType <- gsub("DrugAdr_BbsiProx_separation", "Drug-ADR \n (Shortest path)", select_model_stats$featureType)
select_model_stats$featureType <- gsub("DrugDisease_BbsiProx_separation", "Drug-Disease \n (Shortest path)", select_model_stats$featureType)
select_model_stats$featureType <- gsub("DrgDisAdr_BbsiProx_separation", "Drug-Disease-ADR \n (Shortest path)", select_model_stats$featureType)
...
select_model_stats$featureType <- factor(x = select_model_stats$featureType,
                                         levels = c("Efficacy \n (RWR-FGSEA)", "Safety \n (RWR-FGSEA)", "Combined-efficacy-safety \n (RWR-FGSEA)", 
                                                    "Drug-Disease \n (Shortest path)", "Drug-ADR \n (Shortest path)", "Drug-Disease-ADR \n (Shortest path)"))
...
tiff("OutputFiles/Plots/Publication/panCancer_ModelAccuracy_Test_4.tiff",
     width = 16, height = 4,
     units = "cm", compression = "lzw", res = 1200)

ggplot(select_model_stats, aes(x = featureType, y = value, fill = model)) + #
  ...
  ggtitle(gsub("Cancer", " Cancer", disease)) +
  xlab("Features") + ylab("Accuracy scores") + #
  labs(colour = "Models : ") +
  facet_grid(cols = vars(scoreType)) +
