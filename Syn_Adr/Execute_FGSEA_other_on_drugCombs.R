...
# Script to execute FGSEA on the RWR results using the other library
...
enrichment_lib_1 <- readRDS("InputFiles/Enrichment_analysis_libraries/CTD_Disease2Gene_lib.rds")
names(enrichment_lib_1) <- paste0(names(enrichment_lib_1), "[CTD]")
enrichment_lib_2 <- readRDS("InputFiles/Enrichment_analysis_libraries/DisGeNET_Phenotype2Gene_lib.rds")
names(enrichment_lib_2) <- paste0(names(enrichment_lib_2), "[DisGeNET_Phenotype]")
enrichment_lib_3 <- readRDS("InputFiles/Enrichment_analysis_libraries/DisGeNET_DiseaseGroup2Gene_lib.rds")
names(enrichment_lib_3) <- paste0(names(enrichment_lib_3), "[DisGeNET_DiseaseGroup]")
...
rm(list = c("enrichment_lib_1", "enrichment_lib_2", "enrichment_lib_3"))


fgsea_result <- func_run_FGSEA_on_RWR(rwr_data = rwr_result, 
                                      enrichment_library = enrichment_lib)

fgsea_result_final[["other"]] <- fgsea_result
...
saveRDS(fgsea_result_final, paste0("Syn_Adr/FGSEA_results/fgseaNES_other_", disease, "_", drug_target_type, ".rds"))
